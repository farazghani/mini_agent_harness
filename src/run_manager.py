...
from src.model import LLMResponse, Message , ToolCall
    ...
    def _make_llm(self) -> MockLLM:
        ...
        # once that's built. Kept as MockLLM here so the harness runs standalone.
          return MockLLM(response=[
            LLMResponse(
                type="tool_call",
                content=Message(type="model", text="Let me work that out."),
                tool_call=ToolCall(name="calculator", arguments={"expression": "6 * 7"}),
                tokens_used=15,
            ),
            LLMResponse(
                type="final",
                content=Message(type="model", text="Done — the result is 42."),
                tokens_used=10,
            ),
        ])
    ...
    async def _execute(self, run_id: str) -> None:
        async with self.semaphore:
            try:
                ...
                self.store.append_event(run_id, "run_error", {"error": str(e)})
